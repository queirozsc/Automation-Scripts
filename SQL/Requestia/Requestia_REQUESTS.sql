...
AS
    ...
    IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'Procedimentos' AND TABLE_NAME = 'REQUESTIA_REQUESTS'))
    BEGIN
        TRUNCATE TABLE dbo.REQUESTIA_REQUESTS;
    END

    IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'Procedimentos' AND TABLE_NAME = 'REQUESTIA_QSESSION'))
    BEGIN
        TRUNCATE TABLE dbo.REQUESTIA_QSESSION;
    END

    IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'Procedimentos' AND TABLE_NAME = 'REQUESTIA_QANSWER'))
    BEGIN
        TRUNCATE TABLE dbo.REQUESTIA_QANSWER;
    END

    IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'Procedimentos' AND TABLE_NAME = 'REQUESTIA_QTABLEANSWER'))
    BEGIN
    ...
    IF(@dayName = 'Monday')
    BEGIN 
        IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'Procedimentos' AND TABLE_NAME = 'REQ_WKF_CAPEXNFD'))
        BEGIN
            DROP TABLE dbo.REQ_WKF_CAPEXNFD; -- AUTO CREATED BY sp_EXTRACTCAPEXREQUESTS
        END

        IF (EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'Procedimentos' AND TABLE_NAME = 'REQ_WKF_ATUALIZACAOTISS'))
        BEGIN
    ...
    BEGIN
        ...
        FROM REQUESTIA_REQUESTS R
            LEFT JOIN REQUESTIA_QANSWER A1
                ON R.QSESSIONFORM = A1.QSESSION AND A1.QUESTION = 'CAP_CAPEX_NFD_Projeto'
...
SELECT R.REQUEST CHAMADO
    ...
    , UPPER(R.CLIENT) SOLICITANTE
    , UPPER(R.CURRANAL) ANALISTA
    , UPPER(R.LASTAPPROVUSER) APROVADOR
    , R.MARCA MARCA
    , R.[SIGLA UNIDADE] SIGLA
    , R.[UNIDADE ATENDIMENTO] UNIDADE
FROM REQUESTIA_REQUESTS R
WHERE R.CATEGORY = 'Central de Notas'
    AND (R.MARCA = 'DH' OR R.MARCA = 'IOF')
    AND R.ABERTURA >= GETDATE()-7
ORDER BY 1 DESC
    AND R.PRODUCT = 'Solicitação de CAPEX'
    AND R.PROCESS = 'Registro de NFDs'
    AND R.RSTATUS NOT LIKE '%Cancel%'
...
WHERE R.CATEGORY = 'Facilities'
    ...
    AND R.CLOSED = 1
    AND R.ABERTURA >= '01/01/2019'
    AND R.LASTAPPROVUSER = 'sergio.queiroz'
...
FROM REQUESTIA_REQUESTS R
    ...
    LEFT JOIN REQUESTIA_QANSWER A6
        ON R.QSESSIONFORM = A6.QSESSION AND A6.QUESTION = 'CONTAS_PGTO_Contrato'
WHERE R.CATEGORY = 'Facilities'
    AND R.PRODUCT = 'Servicos'
    AND R.PROCESS = 'Medição de Contratos'
    AND R.CLOSED = 1
    AND R.ABERTURA >= '01/01/2019'--GETDATE()-90
    AND R.LASTAPPROVUSER = 'sergio.queiroz'
...
SELECT TOP 100 R.REQUEST
    ...
    , R.*   
FROM REQUESTIA_REQUESTS R
WHERE R.RESLMINS = R.RESLREMAINS
AND R.CLOSED = 0
--WHERE R.REQUEST = 'CSI086576'
WHERE R.REQUEST IN ('CSI086990', 'CSI086987', 'CSI002183', 'CSI086576', 'CSI076689')
...
SELECT R.RSTATUS
    , CASE 
        WHEN UPPER(R.RSTATUS) LIKE '%AGUARD%ATD%' THEN 'AGUARDANDO ATENDIMENTO' 
        WHEN UPPER(R.RSTATUS) LIKE '%AGUARD%SUBID%' THEN 'AGUARDANDO PUBLICACAO' 
        WHEN UPPER(R.RSTATUS) LIKE '%AGUARD%ATEND%' THEN 'AGUARDANDO ATENDIMENTO' 
        WHEN UPPER(R.RSTATUS) LIKE '%AGUARD%SOL%' THEN 'AGUARDANDO SOLICITANTE' 
        WHEN UPPER(R.RSTATUS) LIKE '%AGUARD%TER%' THEN 'AGUARDANDO TERCEIRO' 
        WHEN UPPER(R.RSTATUS) LIKE '%AGUARD%APROV%' THEN 'AGUARDANDO APROVACAO' 
        WHEN UPPER(R.RSTATUS) LIKE '%APROVADO%' THEN 'APROVADO' 
        WHEN UPPER(R.RSTATUS) LIKE '%CANCEL%' THEN 'CANCELADO' 
        WHEN UPPER(R.RSTATUS) LIKE '%ENCERR%' THEN 'ENCERRADO' 
        WHEN UPPER(R.RSTATUS) LIKE '%EM%ATEND%' THEN 'EM ATENDIMENTO' 
        WHEN UPPER(R.RSTATUS) LIKE '%REPROV%' THEN 'REPROVADO' 
        WHEN UPPER(R.RSTATUS) LIKE '%RET%SOLIC%' THEN 'AGUARDANDO ATENDIMENTO' 
        WHEN UPPER(R.RSTATUS) LIKE '%RET%SOL%' THEN 'AGUARDANDO ATENDIMENTO' 
        WHEN UPPER(R.RSTATUS) LIKE '%REABERTO%' THEN 'AGUARDANDO ATENDIMENTO' 
        WHEN UPPER(R.RSTATUS) LIKE '%DEMANDA%NAO%SOLUC%' THEN 'AGUARDANDO ATENDIMENTO' 
    END SITUACAO
    , COUNT(1)
FROM REQUESTIA_REQUESTS R
WHERE R.REQUEST IN ('CSI071583', 'CSI077930', 'CSI081951', 'CSI085488', 'CSI074064', 'CSI070353')
GROUP BY R.RSTATUS
ORDER BY 3 DESC

SELECT ROW_NUMBER() OVER (ORDER BY R.REQUEST) ITEM
    , *
FROM REQUESTIA_REQUESTS R
WHERE R.REQUEST IN ('CSI071583', 'CSI077930', 'CSI081951', 'CSI085488', 'CSI074064', 'CSI070353')
...
SELECT ROW_NUMBER() OVER (ORDER BY R.REQUEST DESC) ITEM
    , R.REQUEST CHAMADO
    , UPPER(R.PRODUCT) PRODUTO
    , UPPER(R.PROCESS) PROCESSO
    , CONVERT(VARCHAR(10), R.ABERTURA, 103) ABERTURA
...
WHERE R.DATA_ATENDIMENTO BETWEEN CONVERT(VARCHAR, '25/12/2018',103) AND CONVERT(VARCHAR, '17/10/2019', 103)

SELECT TOP 10 *
FROM OFT_CONSULTA_RETORNO R
WHERE R.AGENDADO_PARA_DIA BETWEEN CONVERT(VARCHAR, '25/12/2018', 103) AND CONVERT(VARCHAR, '17/10/2019', 103)

SELECT MIN(R.DATA_ATENDIMENTO)
    , MAX(R.DATA_ATENDIMENTO)
    , MIN(R.AGENDADO_PARA_DIA)
    , MAX(R.AGENDADO_PARA_DIA)
FROM OFT_CONSULTA_RETORNO R
